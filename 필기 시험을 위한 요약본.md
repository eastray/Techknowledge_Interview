...
```
정규표현식과 일치하는 문자열의 부분 문자열을 대체 문자열로 교체한다.
```
...
```
Ubuntu와 CentOS 사용
Ubuntu는 연구실 서버의 용도로 설치하여 사용했으며, 주로 gitlab을 사용하여 소스를 관리 및 공유하는데에 사용했다. CentOS는 수업 조교로 수업에 참여하여 설치하고 Shell을 이용한 수업을 병행했다.
```
...
객체 지향 프로그래밍은 응용 프로그램 및 컴퓨터 프로그램을 설계하기 위해 데이터 필드와 메소드로 구성된 데이터 구조와 상호 작용과 함께 "객체"를 사용하는 프로그래밍 패러다임입니다. 프로그래밍 기술은 데이터 추상화, 캡슐화, 모듈화, 다형성 및 상속과 같은 기능을 포함 할 수 있습니다. 많은 현대 프로그래밍 언어가 이제 객체 지향 프로그래밍을 지원합니다.
```
...
쿠키과 세션의 가장 큰 차이점은 정보를 어디에 저장하는지에 있다. 쿠키는 클라이언트에 저장되며 세션은 서버에 저장된다. 쿠키는 클라이언트의 자원을 사용하며, 세션은 서버의 자원을 사용한다. 쿠키는 비교적 세션보다 속도가 빠르며, 세션은 서버의 처리에 의해서 쿠키보다 속도가 느리다. 쿠키와 세션이 같이 사용되는 이유는 서버 측의 한정된 자원에 세션의 무분별한 사용에 의한 서버 과부하의 문제를 예방하기 위해서이다.
```
